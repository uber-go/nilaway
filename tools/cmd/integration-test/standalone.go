...
func (d *StandaloneDriver) Run(dir string) (Diagnostics, error) {
	cwd, err := os.Getwd()
	if err != nil {
	...
	for _, m := range result {
		...
		for _, d := range diagnostics {
			...
			if len(parts) < 3 {
				return nil, fmt.Errorf("expect at least 3 parts in position string, got %+v", d)
			}

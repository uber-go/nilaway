...
func run(pass *analysis.Pass) ([]annotation.FullTrigger, error) {
	...
	if strings.HasPrefix(pass.Pkg.Path(), config.NilAwayPkgPathPrefix) { //nolint:revive
		...
		functionConfig.EnableAnonymousFunc = true
	}
	...
	for _, file := range pass.Files {
		...
		if functionConfig.EnableAnonymousFunc || (conf.EnableTemplSupport && asthelper.DocContains(file, "Code generated by templ")) {

...
// that we support it
func testDoubleLenCheck(a, b []int) int {
	switch 0 {
	case 1:
		if dummyBool() {
			return a[0] //want "sliced into"
...
// this test makes sure that all forms of range loops correctly produce their ranging expression
// as non-nil - including limiting the scope of that production to within their loop bodies

func testRangeProducesNonnil(a []*int) *int {
	for range a {
